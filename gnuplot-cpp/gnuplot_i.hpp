...
class Gnuplot
{
	...
	Gnuplot& set_linewidth(const double linewidth = 1.0);


    /// turns grid on/off
    inline Gnuplot& set_grid(std::string style="")
	{
		std::stringstream ss;
		ss << "set grid " << style;
		cmd(ss.str());return *this;
	};
    ...
    // -----------------------------------------------
    inline Gnuplot& set_multiplot(const std::string options="")
	{    
		std::stringstream ss;
		ss << "set multiplot " << options; 
		cmd(ss.str());
		return *this;
	};
	...
	template<typename X, typename Y>
	Gnuplot& Gnuplot::plot_xy(const X& x, const Y& y1,const Y& y2,const Y& y3, const std::string &title="");
...
#if defined(WIN32) || defined(_WIN32) || defined(__WIN32__) || defined(__TOS_WIN__)
std::string Gnuplot::m_sGNUPlotFileName = "gnuplot.exe";//"pgnuplot.exe";
...
//	//
//	for (unsigned int i = 0; i < x.size(); i++)
//		tmp << x[i] << " " << y1[i] << " " << y2[i] << " " << y3[i] << std::endl;
//
//	tmp.flush();
//	tmp.close();
//
//
//	//plotfile_xy(name, 1, 2,3,4, title);
//
//	return *this;
//}
